/*
  In the same way as we use flatMap on Option, we can use it on List as well.
  For example, if we want a list with tuples of all combinations of elements of two lists, we write:
 */
val xs = List(1, 2, 3, 4)
val ys = List('a', 'b', 'c')
...
/*
  ...
  'guard'). This is done using an if statement as shown below. Note here that this is NOT an
...
/*
  If you want to keep the list intact rather than draw from it using the '<-', you can use an '='.
  In the following example we use this to get all prime numbers from a list.
