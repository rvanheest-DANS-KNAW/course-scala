...
class WikipediaSuggestion extends Application {
  ...
  override def start(stage: Stage): Unit = {
    ...
    val buttonObs = JavaFxObservable.fromNodeEvents(button, ActionEvent.ACTION).asScala
    val enterObs = JavaFxObservable.fromNodeEvents(textField, ActionEvent.ACTION).asScala
    val textObs = JavaFxObservable.fromObservableValue(textField.textProperty()).asScala
      .doOnNext(text => button.setDisable(StringUtils.isBlank(text)))

    val searchSubscription: Subscription = buttonObs
      .merge(enterObs)
      .withLatestFrom(textObs)((_, text) => text)
      .filterNot(StringUtils.isBlank)
      .distinctUntilChanged
      .flatMap(searchWikipedia)
      .doOnError(_.printStackTrace)
      .doOnError(_ => println("even geduld a.u.b."))
      .retry
      .subscribe(results => {
        items.clear()
        results.foreach(items.add)
      }, e => println(s" something went wrong! $e"))
